...
int AalibInit()
{
	...
	for (i=0;i<2;i++)
	{
		...
		threads[i]=sceKernelCreateThread(c,PlayThread,0x11,0x10000,0,NULL);
		if (threads[i]<0)
		{
...
int ResumeAalibThreads()
{
	int i;
	for (i = 0; i < 8; i++)
	{
		if (hardwareChannels[i])
			sceKernelStartThread(threads[i], sizeof(int),(void*)(&hardwareChannels[i]));
	}
