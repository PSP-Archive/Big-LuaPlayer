...
/**
 * \def G2D_MODULATE_ALPHA(color,alpha)
 * \brief g2dColor modulation.
 *
 * This macro modulates the alpha value of a g2dColor.
 * Input range is from 0 to 255.
 */
#define G2D_MODULATE_ALPHA(color,alpha) \
	((((alpha) * G2D_GET_A(color) / 255) << 24) | ((color) & 0x00FFFFFF))
 ...
 */
/**
 * \enum g2dTriangle_Mode
 * \brief Triangle modes enumeration.
 *
 * Change Triangle draw properties.
 * Can only be used with g2dBeginTriangle.
 */
 /**
 * \enum g2dTFX_Mode
 * \brief TFX modes enumeration.
 *
 * Change TFX properties.
 * Can only be used with g2dSetTexTFX.
 */
/**
 * \enum g2dFlip_Mode
...
typedef enum
{
    G2D_MEMORY_DEFAULT = 0, // Free the complete object and set the pointer to NULL.
	G2D_MEMORY_STRUCT = 1,  // Free the structure and set the pointer to NULL.
	G2D_MEMORY_LINK = 2,    // Remove the object Link and set the pointer to NULL.
	G2D_MEMORY_POINTER = 3, // Set The pointer to NULL.
	G2D_MEMORY_STACK = 4    // Escape the Free function.
} g2dMemory_Type;
...
typedef unsigned int g2dColor32;
typedef unsigned short g2dColor16;
typedef unsigned char g2dColor8;
...
/**
 * \brief Start filling a new display-context.
 *
 * This function will start the GU system.
 * Automatically called by the other functions.
 */
void g2dStart();
 ...
 *
 * This function must be called before object rendering.
 */
int g2dSetDrawTarget(g2dTexture *target);

/**
 * \brief Reset the GU draw target back to the drawBuffer.
 * @return: int - 0 on succes.
 *
 * This function must be called before object rendering.
 * Automatically called by the other functions.
...
/**
 * \brief Begins triangle rendering.
 * @param triangle_mode A g2dTriangle_Mode constant.
 *
 * This function begins object rendering. Calls g2dReset().
 * Three g2dAdd() calls per object.
 * Pass G2D_TRIANGLE_FAN to make a triangle fan (three calls, then one per object).
 */
...
/**
 * \brief Finish current display list and go back to the parent context.
 *
 * This function finish the GU system.
 * Renders the whole display list to the draw buffer.
 * This function dosn't automatically end object drawing. Can be used safely.
 */
void g2dCleanFinish();
 ...
 * Renders the whole display list to the draw buffer.
 * Automatically called by the other functions.
 * This function dosn't automatically end object drawing.
 */
void g2dFinish();
...
/**
 * \brief Wait for vertical blank start.
 * @param n - as integer.
 *
 */
...
/**
 * \brief Clear the transformation stack.
 *
 */
void g2dClearStack();
...
/**
 * \brief Resets the current rotation center.
 *
 * This function must be called during object rendering.
 * Disable the forced rotation center.
 */
void g2dResetRotationCenter();

/**
 * \brief Gets the current screen rotation center.
 * @param rot_x Pointer to save the current rotation x center position.
 * @param rot_y Pointer to save the current rotation y center position.
 *
 * This function must be called during object rendering.
 * Pass NULL if not needed.
 */
...
/**
 * \brief Sets the new screen forced rotation center.
 * @param rot_x The new rotation x center position.
 * @param rot_y The new rotation y center position.
 *
 * This function must be called during object rendering.
 */
...
/**
 * \brief Sets the new forced screen rotation center, relative to the current coordinates.
 * @param rot_x The new  rotation x center position increment.
 * @param rot_y The new  rotation y center position increment.
 *
 * This function must be called during object rendering.
 */
...
/**
 * \brief Set the texture palette.
 * @param plt Pointer of the texture palette.
 * @param type The type of the palette to reset.
 *
 * This function must be called during object rendering.
 */
...
/**
 * \brief Set texture TFX mode.
 * @param G2D_TFX_MODULATE (by default)
                            false to overwrite (better performance).
 *
 * This function must be called during object rendering.
 */
...
/**
 * \brief Set texture Alpha mode.
 * @param true to use the alhpa value (by default).
                            false to overwrite (better performance).
 *
 * This function must be called during object rendering.
 */
